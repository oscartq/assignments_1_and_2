...
while 1
    ...
    for el=1:Nel 
        [Dep(:,:,el), seffmax(:,:,el)] = Constitutive_matrix_3D(stress(:,:,el), seffmax(:,:,el), s0, E, v, E/100);
        m = elemnode(el,:); 
        n = [2*m(1)-1 2*m(1) 2*m(2)-1 2*m(2) 2*m(3)-1 2*m(3) 2*m(4)-1 2*m(4)];
        [Fe, Ke, strain(:,:,el), stress(:,:,el), intpoints(:,:,el)] = plane_4_iso_3D(node(m,1)', node(m,2)', Dep(:,:,el), un(n)', thickness);
        Fint(n) = Fint(n) + Fe;
    end  
