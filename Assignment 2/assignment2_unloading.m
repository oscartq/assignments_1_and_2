...
sigma = 1;
unloading = false;
while sigma(end) >= 1e-3 %We load in xx steps

step=step+1;

if u(end)/L >= 2e-3/L
    if du > 0
        % First step of unloading: set du to 0
        du = 0;
    else
        % Subsequent unloading steps: reverse displacement increment
        du = -0.2e-4;
    end
end
...
% draw some plots
figure(1)
subplot(2,2,1), cla, plot((u/L), sigma*1e-6, 'bo-'), grid on, xlabel('u/L','fontsize',14), ylabel('\sigma [MPa]','fontsize',14);
