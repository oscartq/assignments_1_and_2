function [D,seffmax]=Constitutive_matrix_power_hardening(stress,seffmax,s0,E,v,n, du)
...
for in=1:4 %loop over all 4 integration points     
    ...
    if se >= seffmax(in) && du > 0 % yielding 
           beta = 1;
    else                 % no yielding
           beta = 0; 
    end
    ...
    if beta==1  
        Et=(E/n)./(se./s0).^(n-1); E_Et=E/Et;  % power hardening         
        h=9/(4*E*se^2)*(E_Et-1);
